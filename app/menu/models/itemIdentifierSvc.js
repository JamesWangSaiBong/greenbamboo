...
app.service('ItemIdentifier', function(MenuItem, AdvanceMenuItem) {
	var items = {
		appetizers: [],
		noodles: [],
		lightCourses: [],
	...
	this.identifyMenuItem  = function(menuItems) {
		...
		for(var i=0; i<menuItems.length; i++) {
			if(!menuItems[i].options) {
				...
				if(menuItems[i].type === 'cyo') { menu.cyo.push(new MenuItem(menuItems[i])) };
				...
				if(menuItems[i].type === 'cyo') { menu.cyo.push(new AdvanceMenuItem(menuItems[i])) };
			}			
		};
		return menu;
	}
